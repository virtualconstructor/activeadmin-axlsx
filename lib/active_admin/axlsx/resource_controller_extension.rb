    ...
    module ResourceControllerExtension
      ...
      def index_with_xlsx(options={}, &block)
        index_without_xlsx do |format|
          ...
          format.sizes do
            sizes = active_admin_config.sizes_builder.serialize(collection)
            send_data sizes, :filename => "#{sizes_filename}", :type => Mime::Type.lookup_by_extension(:sizes)
          end
        end
      end
      ...
      def sizes_filename
        "#{resource_collection_name.to_s.gsub('_', '-')}-sizes-#{Time.now.strftime("%Y-%m-%d")}.xlsx"
      end
    end
  end
end
